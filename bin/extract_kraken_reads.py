...
class Tree(object):
    'Tree node.'
    def __init__(self, taxid, level_num, level_type, children=None, parent=None):
        self.taxid = taxid
...
def main():
    ...
    if args.parents or args.children:
        ...
        for line in r_file:
            ...
            while level_num != (prev_node.level_num + 1): # Enrique edits.
                prev_node = prev_node.parent
            #determine correct level ID
            if level_type == '-' or len(level_type) > 1:
                if prev_node.level_type in main_lvls:
                    level_type = prev_node.level_type + '1'
                else:
                    num = int(prev_node.level_type[-1]) + 1
                    level_type = prev_node.level_type[:-1] + str(num)
            #make node
    ...
    for line in k_file:
        ...
        # Enrique: This parts seems to works and finds the read ids that were classfied to the taxid
    ...
    for record in SeqIO.parse(s_file1,filetype):
        ...
        #print("SAVE_READ IDS", save_readids)
        #if "/1" or "/2" in test_id: Enrique: Commented this part out for now, even though I was only using a single end as the input, this section was identifying the
            #test_id = test_id[:-1]
            #print("TEST id", test_id)
        if test_id in save_readids.keys(): # Enrique: Needed to iterate through the keys instead of the whole dictionary.
